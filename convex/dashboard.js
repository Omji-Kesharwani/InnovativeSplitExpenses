...
export const getUserBalances = query({
  handler:async(ctx)=>{
     ...
     const settlements = (await ctx.db.query("settlements").collect()).filter((s)=> !s.groupId && (s.paidByUserId === user._id || s.receivedByUserId === user._id));
...
export const getMonthlySpending = query({
  handler: async(ctx)=>{
   ...
   const currentYear = new Date().getFullYear();
   const startOfYear = new Date(currentYear,0,1).getTime();
   const allExpense = await ctx.db.query("expenses").withIndex("by_date",(q) => q.gte("date",startOfYear)).collect();

   const userExpenses = allExpense.filter((expense)=>
     expense.paidByUserId === user._id ||
  expense.splits.some((split) => split.userId === user._id))
  ...
  userExpenses.forEach((e)=>{
    ...
    const result = Object.entries(monthlyTotals).map(([date,amount])=>({month:parseInt(date),total:amount}));
